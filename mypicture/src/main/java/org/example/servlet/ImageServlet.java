...
public class ImageServlet extends HttpServlet {
    ...
    public static final String IMAGE_DIR = "/root/soft/mypicture";
    ///root/soft/mypicture
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        if(isLogin(req)){
            ...
            try {
                ...
                //上传的图片名可能重复，但md5是唯一的
                //保存文件到服务器本地某个路径
                p.write(IMAGE_DIR+"/"+md5 + name);

                // 图片信息保存在数据库---->后续查询图片列表接口要用
    ...
    private static boolean isLogin(HttpServletRequest req){
            HttpSession session = req.getSession(false);//试图获取session，没有的话不进行创建
            if (session != null &&
